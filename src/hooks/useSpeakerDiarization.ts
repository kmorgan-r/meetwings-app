...
interface BatchSpeakerMap {
  [diarizationLabel: string]: {
    ...
    isAutoGenerated?: boolean; // True if auto-created profile, false if matched existing
  };
}
...
async function matchDiarizationResults(
  ...
  for (const [diarizationLabel, speakerUtts] of speakerUtterances.entries()) {
    // ========== Performance Optimization: Early Exit for High-Confidence Profiles ==========
    // Check if we can skip pitch analysis by reusing a high-confidence profile from previous batches.
      ...
      try {
        ...
        if (
          ...
          batchMap[diarizationLabel] = {
            ...
            isAutoGenerated: false, // Reusing existing high-confidence profile
      ...
      try {
        ...
        if (matchedProfile) {
          ...
          batchMap[diarizationLabel] = {
            ...
            isAutoGenerated: false, // Matched to existing profile
            pitchData,
          };
          ...
          batchMap[diarizationLabel] = {
            ...
            isAutoGenerated: true, // Auto-created profile (unconfirmed)
            pitchData,
          };
        }
    ...
    if (entry) {
      ...
      if (speakerMapping) {
        ...
        updateEntry(entry.timestamp, {
          speakerId: SpeakerIdFactory.profile(speakerMapping.sessionId),
          speakerLabel: speakerMapping.displayName,
          // Confirmed if matched to existing profile (not auto-generated)
          confirmed: !!(speakerMapping.profileId && !speakerMapping.isAutoGenerated),
 ...
 * Note: sessionId is stored without prefix for internal tracking,
 * but converted to profile_X format when exposed as speakerId.
 */
